...
let map_experimental_directives (env : env) (x : CST.experimental_directives) =
  (match x with
  | `ABIE2 tok -> (* "ABIEncoderV2" *) token env tok
  | `SMTC tok -> (* "SMTChecker" *) token env tok
  )
...
let map_pat_f85f0d1 (env : env) (tok : CST.pat_f85f0d1) =
  (* pattern \.[0-9_]+([eE](-)?[0-9_]+)? *) token env tok
...
let map_hex_digit (env : env) (tok : CST.hex_digit) =
  (* pattern [\da-fA-F](_?[\da-fA-F])* *) token env tok

let map_pat_0c477de (env : env) (tok : CST.pat_0c477de) =
  (* pattern "[^\"\\r\\n\\\\]" *) token env tok

let map_string_literal_fragment_single (env : env) (tok : CST.string_literal_fragment_single) =
  (* pattern "[^'\\\\\\n]+|\\\\\\r?\\n" *) token env tok
...
let map_pat_e8d4492 (env : env) (tok : CST.pat_e8d4492) =
  (* pattern [0-9_]+(\.[0-9_]+)?([eE](-)?[0-9_]+)? *) token env tok
...
let map_string_literal_fragment_double (env : env) (tok : CST.string_literal_fragment_double) =
  (* pattern "[^\"\\\\\\n]+|\\\\\\r?\\n" *) token env tok

let map_experimental_directive (env : env) ((v1, v2, v3, v4) : CST.experimental_directive) =
  let v1 = (* "experimental" *) token env v1 in
  let v2 =
    (match v2 with
...
let map_anon_rep_opt___hex_digit_c87bea1 (env : env) (xs : CST.anon_rep_opt___hex_digit_c87bea1) =
  List.map (fun (v1, v2) ->
    ...
    in
    let v2 =
      (* pattern [\da-fA-F](_?[\da-fA-F])* *) token env v2
    in
    todo env (v1, v2)
  ) xs
...
let map_string_ (env : env) (x : CST.string_) =
  (match x with
  | `DQUOT_rep_choice_str_lit_frag_double_DQUOT (v1, v2, v3) ->
      let v1 = (* "\"" *) token env v1 in
      let v2 =
        List.map (fun x ->
          (match x with
          | `Str_lit_frag_double tok ->
              (* pattern "[^\"\\\\\\n]+|\\\\\\r?\\n" *) token env tok
          | `Esc_seq tok -> (* escape_sequence *) token env tok
          )
        ) v2
      ...
      let v3 = (* "\"" *) token env v3 in
      todo env (v1, v2, v3)
  | `SQUOT_rep_choice_str_lit_frag_single_SQUOT (v1, v2, v3) ->
      let v1 = (* "'" *) token env v1 in
      let v2 =
        List.map (fun x ->
          (match x with
          | `Str_lit_frag_single tok ->
              (* pattern "[^'\\\\\\n]+|\\\\\\r?\\n" *) token env tok
          | `Esc_seq tok -> (* escape_sequence *) token env tok
          )
        ) v2
    ...
    let v2 =
      ...
      | `DQUOT_opt_hex_digit_rep_opt___hex_digit_DQUOT (v1, v2, v3) ->
          ...
          let v2 =
            ...
            | Some (v1, v2) ->
                let v1 =
                  (* pattern [\da-fA-F](_?[\da-fA-F])* *) token env v1
                in
...
let rec map_anon_exp_rep_COMMA_exp_opt_COMMA_8dad85b (env : env) ((v1, v2, v3) : CST.anon_exp_rep_COMMA_exp_opt_COMMA_8dad85b) =
...
and map_anon_name_value_rep_COMMA_name_value_opt_COMMA_075ca67 (env : env) ((v1, v2, v3) : CST.anon_name_value_rep_COMMA_name_value_opt_COMMA_075ca67) =
  let v1 = map_name_value env v1 in
  let v2 =
    List.map (fun (v1, v2) ->
      let v1 = (* "," *) token env v1 in
      let v2 = map_name_value env v2 in
      todo env (v1, v2)
    ) v2
  in
  let v3 =
...
and map_call_arguments (env : env) ((v1, v2, v3) : CST.call_arguments) =
  ...
  let v2 =
    (match v2 with
    | `Opt_exp_rep_COMMA_exp_opt_COMMA opt ->
        (match opt with
        | Some x ->
            map_anon_exp_rep_COMMA_exp_opt_COMMA_8dad85b env x
        | None -> todo env ())
    | `LCURL_opt_name_value_rep_COMMA_name_value_opt_COMMA_RCURL (v1, v2, v3) ->
        let v1 = (* "{" *) token env v1 in
        let v2 =
          (match v2 with
          | Some x ->
              map_anon_name_value_rep_COMMA_name_value_opt_COMMA_075ca67 env x
          | None -> todo env ())
        in
        let v3 = (* "}" *) token env v3 in
        todo env (v1, v2, v3)
...
and map_name_value (env : env) ((v1, v2, v3) : CST.name_value) =
  let v1 =
    (* pattern [a-zA-Z$_][a-zA-Z0-9$_]* *) token env v1
  in
  let v2 = (* ":" *) token env v2 in
...
and map_return_type_definition (env : env) ((v1, v2) : CST.return_type_definition) =
...
let map_variable_declaration_tuple (env : env) (x : CST.variable_declaration_tuple) =
  (match x with
  | `Var_LPAR_opt_opt_id_rep_COMMA_opt_id_opt_COMMA_RPAR (v1, v2, v3, v4) ->
      ...
      let v4 = (* ")" *) token env v4 in
      todo env (v1, v2, v3, v4)
  | `LPAR_opt_rep_COMMA_var_decl_rep_COMMA_opt_var_decl_RPAR (v1, v2, v3, v4, v5) ->
      let v1 = (* "(" *) token env v1 in
      let v2 =
        (match v2 with
        | Some xs -> List.map (token env (* "," *)) xs
        | None -> todo env ())
      in
      ...
      let v4 =
        List.map (fun (v1, v2) ->
          ...
          let v2 =
            (match v2 with
            | Some x -> map_variable_declaration env x
            | None -> todo env ())
          in
          todo env (v1, v2)
        ) v4
...
let map_variable_declaration_statement (env : env) ((v1, v2, v3) : CST.variable_declaration_statement) =
  let v1 =
    (match v1 with
    | `Var_id (v1, v2) ->
        let v1 = (* "var" *) token env v1 in
    ...
    | `Var_decl_tuple x -> map_variable_declaration_tuple env x
    )
  in
  let v2 =
    (match v2 with
...
and map_statement (env : env) (x : CST.statement) =
  ...
  | `Unch_blk (v1, v2) ->
      ...
      let v2 = map_block_statement env v2 in
      todo env (v1, v2)
...
let map_fallback_receive_definition (env : env) ((v1, v2, v3, v4, v5, v6) : CST.fallback_receive_definition) =
  let v1 =
    (match v1 with
